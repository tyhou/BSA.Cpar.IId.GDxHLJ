rm(list = ls())
d("D:/Genome-seq/IId-cross-BSA-24-10-01/pca-dip//")
eigvec <- read.table("BSA2-GKO-noPRM_dip_Marker_1065.eigenvec", header = F, stringsAsFactors = F)
write.table(eigvec[2:ncol(eigvec)], file = "plink.eigenvector.xls", sep = "\t", row.names = F, col.names = T, quote = F)

eigval <- read.table("BSA2-GKO-noPRM_dip_Marker_1065.eigenval", header = F)
...
poptable <- read.table("BSA2-GKO-noPRM-pca.pop.txt", header = T, comment.char = "")
...
pca.data$DPI <- factor(pca.data$DPI,levels=c("HLJ","GD","F1","6","12","18","24","30","36","42","48","54","60"))
...
p <- ggplot(pca.data, aes(x=PC1,y=PC2,color=DPI))+
  ...
  #fac(x=paste("PC 1(60.1%)", sep=""),
       y=paste("PC 2(14.9%)", sep=""))
p

ggsave("PCA-BSA2-GKO-noPRM.pdf", plot = p, device = "pdf", dpi = 300,width = 8,height = 6)
ggsave("PCA-BSA2-GKO-noPRM.png", plot = p, device = "png", dpi = 300,width = 8,height = 6)
